...
class rcpp_distribution_caller {
    ...
    struct rtype : public std::integral_constant<int, -1> {
    };  // not implemented

    template <typename _T>
    struct rtype<_T,
                 std::enable_if_t<std::is_same_v<bool, std::remove_cv_t<_T>>>>
    ...
    using value_type =
        typename __value_type<typename dist_t::result_type>::type;
