...
context("r_sort") {
    ...
    for (unsigned i = 0; i < unsorted_vectors.size(); ++i) {
        ...
        test_that("Rf_revsort - " + std::to_string(i)) {
            ...
            std::generate(a_expected.begin(), a_expected.end(),
                          [&unsorted_values, i = 0]() mutable {
                              return static_cast<double>(
                                  std::get<0>(unsorted_values[i++]));
                          });
            std::generate(
            ...
            rmolib::algorithm::r_sort(unsorted_values.begin(),
                                      unsorted_values.end(), comp);
            ...
            std::generate(
                ib.begin(), ib.end(), [&unsorted_values, i = 0]() mutable {
                    return static_cast<int>(std::get<1>(unsorted_values[i++]));
                });
