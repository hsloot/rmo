...
unit tests. Nevertheless, it can happen to run into unexpected results for
certain parameterisations in high dimensions.
The reason for this  are numerical  issues
with very small and very large numbers. We tried to program defensively to avoid
these problems, but if the dimension is high enough and the parameterisation
...
- __Version 0.x__: Refactor the `arnold_distribution` and the
