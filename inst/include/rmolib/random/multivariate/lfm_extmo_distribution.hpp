...
class lfm_extmo_distribution {
   ...
   public:
    ...
    class param_type {
       public:
        ...
        explicit param_type(const std::size_t dim,
            ...
            : dim_{dim},
              ...
              compound_poisson_parm_{
                  std::make_pair(intensity_parm, jump_parm)} {
            __validate_input();
        }
        ...
        explicit param_type(_LFMExtMOParamType&& parm)
            : param_type{parm.dim(), parm.killing(), parm.drift(),
                         parm.intensity(), parm.jump_param()} {}
        ...
        friend bool operator==(const param_type& lhs, const param_type& rhs) {
            return lhs.dim_ == rhs.dim_ &&
                   lhs.killing_parm_ == rhs.killing_parm_ &&
       ...
       private:
        ...
        void __validate_input() const {
            ...
            if (drift < 0)
                throw std::domain_error("drift must not be negative");

            if (0 == killing && 0 == drift && (0 == intensity || 0 == jump_max))
                throw std::domain_error(
                    "Lévy subordinator must not be constant");
    ...
    template <typename _LFMExtMOParamType,
              std::enable_if_t<
                  !std::is_convertible_v<_LFMExtMOParamType,
                                         lfm_extmo_distribution> &&
                      !std::is_convertible_v<_LFMExtMOParamType, param_type> &&
                      is_lfm_extmo_param_type_v<_LFMExtMOParamType>,
                  int> = 0>
    ...
    void operator()(_Engine&& engine, const param_type& parm, _Container& out) {
        ...
        for (const auto& [index, barrier] : sorted_barriers_with_indeces) {
            while (value < barrier) {
                ...
                } else if (value + (next_time - time) * parm.drift_ >=
                           barrier) {
