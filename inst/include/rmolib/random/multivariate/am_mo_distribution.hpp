...
class am_mo_distribution {
   public:
       ...
       public:
        ...
        auto lambda() const {
            ...
            std::transform(out.cbegin(), out.cend(), out.begin(),
                           [total_intensity = poisson_parm.lambda()](
       ...
       private:
        ...
        void __init_empty() {
            compound_poisson_parm_ =
                std::make_pair(exponential_parm_t{_RealType{1}},
        ...
        void __init(_ForwardIterator first, _ForwardIterator last,
            ...
            compound_poisson_parm_ = std::make_pair(std::move(poisson_parm),
    ...
    void operator()(_Engine&& engine, const param_type& parm, _Container& out) {
        ...
        while (dead != all) {
            ...
            state = __compound_poisson_process(
                engine, parm.compound_poisson_parm_, state);

            for (std::size_t i = 0; i < dim; ++i)
                if (internal::is_within(i, alive_before_transition))
                    out[i] = time;
        }
    }
   ...
   private:
    using compound_poisson_parm_t =
        typename param_type::compound_poisson_parm_t;
