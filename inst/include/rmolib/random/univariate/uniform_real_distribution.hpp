...
//! true, if _T can be used to construct uniform_real_distribution<>::param_type
template <typename _T>
...
class uniform_real_distribution {
   public:
    ...
    class param_type {
       ...
       private:
        ...
        void __validate_input(const _RealType lower,
            ...
            if (!(is_finite(lower) && is_finite(upper)))
                throw std::domain_error(
                    "lower and upper must be in (-infty, infty)");
        ...
        static_assert(
            ...
            "Class template rmolib::random::uniform_real_distribution<> must "
            "be "
    ...
    result_type operator()(_Engine&& engine, const param_type& parm) {
        return parm.lower_ +
               parm.length_ *
                   internal::unit_uniform_real_distribution<_RealType>(
