...
__Proof:__
The statement is true for \( j = 1 \) with \( c_{0} = 1 \) and \( c_{k} = 0,\ k > 0 \).
This representation is also unique.
Assume now that the statement is true for all natural numbers smaller than \( j \).
If \( j \) is divisible by \( 2 \), then there exists by induction assumption a sequence \( \tilde{c} = {\{ \tilde{c}_{k} \}}_{k \in \mathbb{N}} \) for \( j / 2 \) with
\[
    j / 2
        = \sum_{k=0}^{\infty}{
            \tilde{c}_{k} 2^{k}
...
If \( j \) is not divisible by \( 2 \), then \( j-1 \) is and there exists by induction assumption a sequence \( \tilde{c} = {\{ \tilde{c}_{k} \}}_{k \in \mathbb{N}} \) for \( (j-1)/2 \) with
\[
    (j-1)/2
        = \sum_{k=0}^{\infty}{
            \tilde{c}_{k} 2^{k}
...
Since each summand with a positive \( c_{k} \) contributes an amount bigger or equal to \( 1 \) to the sum, it is clear that all but finitely many \( c_{k} \) must be equal to zero.


## Uniqueness of the binary representation

It can be proven by induction that the binary representation is unique.
To see this, consider two sequences \( \hat{c} = {\{ \hat{c}_{k} \}}_{k \in \mathbb{N}} , c = {\{ c_{k} \}}_{k \in \mathbb{N}} \in {\{ 0 , 1 \}}^\mathbb{N} \) with
\[
    \sum_{k=0}^{\infty} c_{k} 2^{k}
        = \sum_{k=0}^{\infty} \hat{c}_{k} 2^{k} .
\]
It is easy to see, that this number is even if and only if \( c_{0} = 0 \) and odd if and only if \( c_{0} = 1 \).
Hence, we can w.l.o.g. assume that \( c_{0} = \hat{c}_{0} = 0 \), i.e., the number is even.
