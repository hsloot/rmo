...
test_that("`is_nonnegative_vector` assertion works as intended", {
  ...
  expect_error(assert_that(is_nonnegative_vector(-1)),
    regexp=sprintf(ERR_X_NOT_Y, "*", "non-negative vector"))
  expect_error(assert_that(is_nonnegative_vector(c(rep(1, 5), -1))),
    regexp=sprintf(ERR_X_NOT_Y, "*", "non-negative vector"))
})
...
test_that("`is_nonzerovector` assertion works as intended", {
  ...
  expect_error(assert_that(is_nonzero_vector(0)),
    regexp=sprintf(ERR_X_NOT_Y, "*", "non-zero vector"))
  expect_error(assert_that(is_nonzero_vector(rep(0, 5))),
    regexp=sprintf(ERR_X_NOT_Y, "*", "non-zero vector"))
})
...
test_that("`is_lfm_cpp_mo_parameter` assertion works as intended", {
  d <- 2L
  expect_true(assert_that(is_lfm_cpp_mo_parameter(d, 1, 0, 0, "rexp", list("rate"=2))))
  expect_true(assert_that(is_lfm_cpp_mo_parameter(d, 0, 0, 1, "rposval", list("value"=1))))
  expect_true(assert_that(is_lfm_cpp_mo_parameter(d, 0, 1, 0, "rposval", list("value"=1))))
  expect_true(assert_that(is_lfm_cpp_mo_parameter(d, 1, 1, 2, "rexp", list("rate"=2))))
