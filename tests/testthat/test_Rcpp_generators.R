...
test_that("ExpGenerator works as expected", {
  ...
  args <- list("rate" = 0)
  expect_equal_rn_generation(
    "mo_internal__exponential", "rexp_rate_is_zero",
    args, n, use_seed)
  ...
  args <- list("rate" = Inf)
  expect_equal_rn_generation(
    "mo_internal__exponential", "rexp_rate_is_inf",
    args, n, use_seed)
})
...
test_that("FixedDblGenerator works as expected", {
  args <- list("value" = 1)
  expect_equal_rn_generation(
    "rposval", "mo_internal__deterministic",
    args, n, use_seed)
})
...
test_that("CountReplaceGenerator works as expected", {
  ...
  expect_equal_rn_generation(
    "mo_internal__rdiscrete", "sample_count_replace_base_0",
    args, n, use_seed)
